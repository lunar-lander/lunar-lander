  ...
  const handleSendMessage = async (
    ...
    try {
      ...
      if (conversationMode === ConversationModeType.ROUND_ROBIN) {
        ...
        console.log(
          `Starting Round Robin conversation mode with ${modelIds.length} models`
        );
        ...
        messagePromises = orderedModelIds.map((modelId, index) => {
          ...
          console.log(
            `Creating message ${assistantMessageId} for model ${modelId} (round robin position: ${
              index + 1
            })`
          );
            ...
            setTimeout(() => {
              ...
              if (index === 0) {
                try {
                  ...
                  console.error(
                    `Error setting up API call for ${modelId}:`,
                    error
                  );
                  setStreamingMessageIds((prev) =>
                    prev.filter((id) => id !== assistantMessageId)
      ...
      Promise.all(messagePromises)
        .then((messageIds) => {
          console.log(
            `Successfully added ${messageIds.length} messages to chat ${chatId}`
          );
          ...
          if (updatedChat) {
            console.log(
              `Chat ${chatId} now has ${updatedChat.messages.length} messages`
            );
            setChat(updatedChat);
            ...
            if (isFirstMessage && summaryModelId) {
              ...
              setTimeout(() => {
                console.log(
                  `Setting up summary generation wait for chat ${chatId}`
                );
                ...
                const waitForFirstResponse = setInterval(() => {
                  ...
                  if (streamingMessageIds.length === 0) {
                    ...
                    console.log(
                      `Still waiting for ${streamingMessageIds.length} responses to complete before summary`
                    );
                  }
                }, 1000);
              }, 500);
            ...
            console.error(
              `Failed to retrieve updated chat ${chatId} after adding messages`
            );
          }
        })
        .catch((error) => {
  ...
  const generateChatSummary = async (chatId: string) => {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `Refreshing chat after summary with ${updatedChat.messages.length} messages`
        );
        setChat(updatedChat);
      } else {
  ...
  const callLLMApi = async (
    ...
    try {
      ...
      if (conversationMode === ConversationModeType.ISOLATED) {
        ...
        filteredMessages = sortedChatMessages;
        console.log(
          `CUSTOM mode: Using default behavior (all messages visible)`
        );
      }

      console.log(`Conversation mode: ${conversationMode}`);
      console.log(
        `Filtered ${chatMessages.length} total messages to ${filteredMessages.length} messages for model ${modelId}`
      );
      console.log(
